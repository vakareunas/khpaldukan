...
class ListScreen extends Component {
  ...
  componentWillMount() {
    ...
    axios.get(`${base}get-subchild-category/${this.props.navigation.getParam('subcatid')}`)
          .then(response => {
...
<Modal
        ...
        <Header style={{height: 0}} hasTabs/>
          <Tabs
            ...
            renderTabBar={()=> <ScrollableTab tabsContainerStyle={{backgroundColor: colors.gray}}
            underlineStyle={{backgroundColor: colors.primary}}
            />}>
            {
              this.state.products.map(prod => {
                return <Tab heading={prod.subcattitle}
                        style={{color: colors.third, fontWeight: 'bold'}}
                        activeTabStyle={{backgroundColor: colors.primary}}
                        textStyle={{color: colors.third}}
                        tabStyle={{backgroundColor: colors.gray}} key={prod.cid}>
                        <Content>
                            {
                              prod.products.map(v => {
